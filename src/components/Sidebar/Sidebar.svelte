...
<div class="{state?"md:w-60 max-md:w-full absolute max-md:bg-opacity-100 z-50":"min-w-[5rem] w-20 z-50 fixed max-md:w-full"} max-md:bottom-0 max-md:h-20 max-md:bg-card-indigo text-white font-sans h-screen md:sticky ease-in-out duration-700 bg-card-indigo bg-opacity-50 md:rounded-tl-xl">
    ...
    <div class="{state?"sm:pl-5":""} flex flex-col max-md:flex-row max-md:items-center sm:py-2 sm:space-y-3 w-full h-full max-md:border-t-2 max-md:border-white/10">
        ...
        <Nav path={"dashboard"} bind:barState={state} content="Dashboard">
        ...
        {/if}
        {#if user.role === "Manager"}
        <Nav path={"departments"} bind:barState={state} content="Departments">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
        ...
        {/if}
        {#if user.role === "Manager"}
        <Nav path={"exports"} bind:barState={state} content="Exports">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
        ...
        {/if}
        {#if user.role === "Admin"}
        <Nav path={"events"} bind:barState={state} content="Events">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
